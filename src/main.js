...
async function fetchArrayBuffer(path) {
  ...
  const res = await fetchWithRetry(path, {}, { timeout: 8000, retries: 2, backoff: 200 });
...
async function fetchWithRetry(input, init = {}, { timeout = 8000, retries = 3, backoff = 250 } = {}){
  let attempt = 0;
  let lastErr = null;
  while (attempt <= retries) {
    ...
    try {
      ...
      clearTimeout(timer);
      lastErr = e;
      debugLog('fetchWithRetry: attempt failed', { attempt, input, err: String(e) });
...
function showErrorOverlay(message, details, retryCallback){
  try {
    ...
    overlay.focus?.();
  } catch (e) { debugLog('showErrorOverlay failed', e); }
}
...
function hideErrorOverlay(){
  ...
  } catch (e) { debugLog('hideErrorOverlay failed', e); }
}
...
while (true) {
  try {
    ...
    debugLog('main: data load failed', err && err.message ? err.message : String(err));
    console.error('Data load failed:', err);
    // Show overlay and await user retry
    let resolveRetry;
    const retryPromise = new Promise(res => { resolveRetry = res; });
    showErrorOverlay('Failed to load data. Check your network and try again.', String(err), async () => { resolveRetry(); });
    await retryPromise;
    await sleep(200);
...
// Camera setup
const camera = new THREE.PerspectiveCamera(state.fov, window.innerWidth / window.innerHeight, state.near, state.far);
...
let positions, ids, idToName, jumps, indexOf, stationSystemSet, blackHoleSystemSet, dataRelease;
...
setupSearch({ idToName }, (systemName) => {
  const system = Object.values(data.idToName).find(s => s.name === systemName);
  if (system) {
...
async function init() {
  ...
  ({ positions, ids, idToName, jumps, indexOf, stationSystemSet, blackHoleSystemSet, dataRelease } = data);
  
  // --- Build starfield -------------------------------------------------
  const starfield = makeStarfield(positions, ids, stationSystemSet, blackHoleSystemSet);
  scene.add(starfield);
  ...
  if (activeRoute && activeRoute.waypoints && activeRoute.waypoints.length > 0) {
    ...
    const routeLines = makeRouteLines(activeRoute.waypoints, indexOf, positions);
    if (routeLines) {
  ...
  camera.position.set(center[0], center[1] + radius * 1.5, center[2] + radius * 2);
  ...
  } catch (e) { debugLog('init: failed to read focus param', e); }
...
init().catch(e => { debugLog('init error', e); console.error(e); });