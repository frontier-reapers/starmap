...
describe("Bounds computation", () => {
  ...
  it("should handle negative coordinates", () => {
    const positions = new Float32Array([-10, -10, -10, 10, 10, 10]);
...
describe("Route bitpacking decoder", () => {
  ...
  it("should validate route data header", () => {
    ...
    // Too short
    expect(() => validateHeader(new Uint8Array([1, 2]))).toThrow(
      "Route data too short",
    );

    // Wrong version
    expect(() => validateHeader(new Uint8Array([2, 20, 0, 3]))).toThrow(
      "Unsupported route version",
    );

    // Invalid k
    expect(() => validateHeader(new Uint8Array([1, 0, 0, 3]))).toThrow(
      "Invalid route bit width",
    );
    expect(() => validateHeader(new Uint8Array([1, 31, 0, 3]))).toThrow(
      "Invalid route bit width",
    );
  });
