...
def validate_assets(
    ids, positions, jumps, station_ids, black_hole_ids, valid_system_ids
):
    ...
    if pos_count != id_count:
        issues.append(
            f"positions count ({pos_count}) does not match ids count ({id_count})"
        )
    ...
    invalid_jumps = [
        (a, b)
        for (a, b) in jumps
        if a not in valid_system_ids or b not in valid_system_ids
    ]
    if invalid_jumps:
        sample = invalid_jumps[:5]
        issues.append(
            f"{len(invalid_jumps)} jumps reference missing systems (examples: {sample})"
        )
    ...
    if invalid_station_ids:
        ...
        issues.append(
            f"{len(invalid_station_ids)} station system IDs are missing from systems (examples: {sample})"
        )

    # Black hole systems must exist
    invalid_black_hole_ids = [
        sid for sid in black_hole_ids if sid not in valid_system_ids
    ]
    if invalid_black_hole_ids:
        sample = invalid_black_hole_ids[:5]
        issues.append(
            f"{len(invalid_black_hole_ids)} black hole IDs are missing from systems (examples: {sample})"
        )
...
def main():
    ...
    ap.add_argument(
        "--out", default="./public/data", help="Output directory for asset files"
    )
    # Optional overrides
    ...
    ap.add_argument(
        "--black-holes-csv",
        help="CSV file with one or more system IDs (one per line or comma-separated) for black holes",
    )
    ap.add_argument(
        "--black-holes-table",
        help="SQLite table name that contains black hole system IDs",
    )
    ap.add_argument(
        "--black-holes-col",
    ...
    if not systems_table:
        raise SystemExit(
            "Could not find systems table; use --systems-table to specify."
        )

    sys_cols = get_cols(cur, systems_table)
    id_col = (
        args.sys_id_col
        or find_col(sys_cols, ["id", "system_id", "solarsystemid", "smart_object_id"])
        or sys_cols[0]
    )
    name_col = args.sys_name_col or find_col(sys_cols, ["name", "system_name", "label"])
    x_col = args.sys_x_col or find_col(
        sys_cols, ["x", "posx", "position_x", "world_x", "centerx"]
    )
    y_col = args.sys_y_col or find_col(
        sys_cols, ["y", "posy", "position_y", "world_y", "centery"]
    )
    z_col = args.sys_z_col or find_col(
        sys_cols, ["z", "posz", "position_z", "world_z", "centerz"]
    )
    ...
    if duplicates:
        ...
        print(
            f" - {len(duplicates)} duplicate IDs (examples: {sample})", file=sys.stderr
        )
        raise SystemExit(1)
    ...
    try:
        cur.execute(
            "SELECT DISTINCT solarSystemId FROM NpcStations WHERE solarSystemId IS NOT NULL"
        )
        for row in cur.fetchall():
            station_systems.add(int(row[0]))
        print(
            f"Found {len(station_systems)} systems with NPC stations",
            file=__import__("sys").stderr,
        )
    except Exception as e:
        print(
            f"Warning: Could not load NPC stations: {e}", file=__import__("sys").stderr
        )
    ...
    if jumps_table:
        ...
        if source_col and target_col:
            cur.execute(
                f"SELECT {source_col} as s, {target_col} as t FROM {jumps_table}"
            )
    ...
    if args.black_holes_csv:
        ...
        try:
            ...
            bh_col = (
                args.black_holes_col
                or find_col(
                    bh_cols, ["solarSystemId", "system_id", "id", "solar_system_id"]
                )
                or bh_cols[0]
            )
            cur.execute(
                f"SELECT DISTINCT {bh_col} FROM {bh_table} WHERE {bh_col} IS NOT NULL"
            )
            for r in cur.fetchall():
                black_hole_ids_values.append(int(r[0]))
        except Exception as e:
            print(
                f"Error reading black holes table '{args.black_holes_table}': {e}",
                file=sys.stderr,
            )
            raise SystemExit(1)
    else:
    ...
    manifest = {
        ...
        "schema": {
            ...
            "jumps.bin": {
                "type": "Uint32Array",
                "components": 2,
                "meaning": "pairs of system IDs [a,b]",
            },
            "systems_with_stations.bin": {
                "type": "Uint32Array",
                "meaning": "IDs of systems with NPC stations",
            },
            "systems_black_holes.bin": {
                "type": "Uint32Array",
                "meaning": "IDs of black hole systems",
            },
        },
        "coord_system": {
            "units": "lightyears",
